...
cp -R ~/GitHub/ansible-role-sql-developer /tmp/roles/kurron.sql-developer
...
CMD="docker run --env DISPLAY=unix${DISPLAY} \
                --env HOME=${HOME_DIR} \
                --env DATAGRIP_JDK=/usr/lib/jvm/azul-jdk-8 \
                ...
                kurron/ansible-role-sql-developer:latest bash"
echo $CMD
$CMD
